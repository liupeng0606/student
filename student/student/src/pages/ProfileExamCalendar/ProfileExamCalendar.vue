    ...
    <div class="exam_calendar_detail" v-if="examCalendar.length">
      <ul class="exam_calendar_detail_ul" v-for="(item, index) in examCalendar" :key="index">
        <li class="exam_calendar_time_li">
          <span class="exam_calendar_time">推送时间：{{item.noticeCreateTime | date-format}}</span>
        </li>
        <li class="message_li">
          <viewer>
          ...
          <span>
              <i class="iconfont iconxiazai41">详情</i>
              <br>
              您好，我是{{item.teaName}}，教工号是{{item.tno}}。
            <br>推送内容如下：
