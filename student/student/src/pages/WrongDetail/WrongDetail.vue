    ...
    <div class="paper_container" v-show="showPaperContainer">
      ...
      <section class="que" v-for="(item, index) in singleQueList" :key="'single'+ item.singleId"
        ...
        <div class="content">
          ...
          <div class="answer_row">答案解析：<span class="correct_answer">{{item.explain || '暂无解析，给个解析呗'}}</span></div>


          <div style="margin-top: 100px;">
            <div style="float:left;margin-left:30px;">
              <mt-button size="small" type="primary" @click.native="t777('single', item.singleId, item.explain)">个人知识地图</mt-button>
            </div>
            <div style="float:right; margin-right:30px;">
              <mt-button size="small" type="primary" @click.native="t666('single', item.singleId, item.explain)">课程知识地图</mt-button>
            </div>
          </div>
      ...
      <section class="que" v-for="(item, index) in multipleQueList" :key="'multiple'+ item.multipleId"
        ...
        <div class="content">
          ...
          <div class="answer_row">你的答案：<span :class="[item.isCorrect == '1' ? 'correct_answer' : 'your_answer']">{{item.stuAnswer || '你太优秀了，该题无作答'}}</span></div>
          <div class="answer_row">是否正确：<span :class="[item.isCorrect == '1' ? 'correct_answer' : 'your_answer']">{{item.isCorrect == '1' ? '正确' : '错误'}}</span></div>
          <div class="answer_row answer_explain">答案解析：<span class="correct_answer">{{item.explain || '暂无解析，给个解析呗'}}</span></div>


          <div style="margin-top: 100px;">
            <div style="float:left;margin-left:30px;">
              <mt-button size="small" type="primary" @click.native="t777('multiple', item.multipleId, item.explain)">个人知识地图</mt-button>
            </div>
            <div style="float:right; margin-right:30px;">
              <mt-button size="small" type="primary" @click.native="t666('multiple', item.multipleId, item.explain)">课程知识地图</mt-button>
            </div>
          </div>
      ...
      <section class="que" v-for="(item, index) in judgeQueList" :key="'judge'+ item.judgeId"
        ...
        <div class="content">
          ...
          <div style="margin-top: 100px;">
            <div style="float:left;margin-left:30px;">
              <mt-button size="small" type="primary" @click.native="t777('judge', item.judgeId, item.explain)">个人知识地图</mt-button>
            </div>
            <div style="float:right; margin-right:30px;">
              <mt-button size="small" type="primary" @click.native="t666('judge', item.judgeId, item.explain)">课程知识地图</mt-button>
            </div>
          </div>
      ...
      <section class="que" v-for="(item, index) in fillQueList" :key="'fill'+ item.fillId"
        ...
        <div class="content">
          ...
          <div style="margin-top: 100px;">
            <div style="float:left;margin-left:30px;">
              <mt-button size="small" type="primary" @click.native="t777('fill', item.fillId, item.explain)">个人知识地图</mt-button>
            </div>
            <div style="float:right; margin-right:30px;">
              <mt-button size="small" type="primary" @click.native="t666('fill', item.fillId, item.explain)">课程知识地图</mt-button>
            </div>
          </div>
        </div>
      </section>
      ...
      <!--上一题和下一题按钮-->
      <div class="paper_button" style="margin-bottom: 15px">
        ...
        <mt-button type="primary" @click.native="nextItem" :disabled="currentIndex == queNumInfo.totalNum-1">
          {{currentIndex == queNumInfo.totalNum-1 ? '到底了' : '下一题'}}
        </mt-button>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      t666(ttype, typeid, con){

        sessionStorage.setItem("con", JSON.stringify(con))

        var ccc = sessionStorage.getItem("con")

        console.log(ccc)

        window.location.href = 'http://62.234.194.53:666?q_type='+ttype+"&q_id="+typeid+"&con="+con;

      },

      t777(ttype, typeid, con){
        ...
        window.location.href = 'http://62.234.194.53:777?q_type='+ttype+"&q_id="+typeid+"&con="+con;
